// JpegFile.cpp: implementation of the CJpegFile class.
//
//////////////////////////////////////////////////////////////////////
...
//	read a JPEG file
//
BYTE * CJpegFile::JpegBufferToRGB(BYTE*pDataBuffer,UINT nSize,UINT *width,UINT *height)
{
	...
	*/
	...
	XXX_JPEG_BUFFER filebuffer2;
	filebuffer2.buff = pDataBuffer;
	filebuffer2.next_byte_to_write = 0;
	if (filebuffer2.buff == NULL) {
		return 0;
	}

    filebuffer2.buffsize = nSize;
	...
	jpeg_stdio_srcf(&cinfo, &filebuffer2);
...
BYTE * CJpegFile::JpegFileToRGB(CString fileName,
							   UINT *width,
							   UINT *height)

{
	...
	*/
	...
	if ((infile = __fopen(fileName, _T("rb"))) == NULL) {
		sprintf(buf, "JPEG :\nCan't open %s\n", fileName);
//		AfxMessageBox(buf);
		return NULL;
	}
...
BOOL CJpegFile::GetJPGDimensions(CString fileName,
								UINT *width,
								UINT *height)
...
BOOL CJpegFile::RGBToJpegFile(CString fileName, 
							BYTE *dataBuf,
							UINT widthPix,
							UINT height,
							BOOL color, 
...
//			AfxMessageBox("Memory error");
	...
	if ((outfile = __fopen(fileName, _T("wb"))) == NULL) {
		char buf[250];
...
LONG CJpegFile::RGBToJpegBuffer(char **filebuffer, BYTE *dataBuf, UINT widthPix, UINT height, BOOL color, int quality)
{
	if (dataBuf==NULL)
		return -1;
	if (widthPix==0)
...
//			AfxMessageBox("Memory error");
	...
	/* Step 2: specify data destination (eg, a file) */
	/* Note: steps 2 and 3 can be done in either order. */
	XXX_JPEG_FILE_BUFFER filebuffer1;
	filebuffer1.buff = (unsigned char *)malloc(JPEG_BUFFERSIZE );
	filebuffer1.next_byte_to_write = 0;
	if (filebuffer1.buff == NULL) {
		jpeg_destroy_compress(&cinfo);
		return 0;
	}
	
	jpeg_stdio_destf(&cinfo, &filebuffer1);
...
/* Now use the library's routine to set default compression parameters.
  ...
  * filebuffer = (char*)filebuffer1.buff;

  return filebuffer1.next_byte_to_write;
