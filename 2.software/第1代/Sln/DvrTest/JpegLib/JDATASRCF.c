...
typedef struct {
  ...
  XXX_JPEG_BUFFER * infile;		/* source stream */
  JOCTET * buffer;		/* start of buffer */
  boolean start_of_file;	/* have we gotten any data yet? */
} my_source_mgrf;

typedef my_source_mgrf * my_src_ptrf;
...
{
  my_src_ptrf src = (my_src_ptrf) cinfo->src;
...
GLOBAL(void)
jpeg_stdio_srcf (j_decompress_ptr cinfo, XXX_JPEG_BUFFER * inbuffer)
{
  my_src_ptrf src;
  ...
  if (cinfo->src == NULL) {	/* first time for this JPEG object? */
    cinfo->src = (struct jpeg_source_mgr *)
      (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT,
				  SIZEOF(my_source_mgrf));
    src = (my_src_ptrf) cinfo->src;
    src->buffer = (JOCTET *)
      (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, JPOOL_PERMANENT,
